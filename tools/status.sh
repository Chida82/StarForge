...
for name in $(registry_names); do
    ...
    base="$(git -C "$dir" rev-parse --short=7 "$(child_committed_base_sha "$name")")"
    ...
    if child_sync_in_progress "$name"; then
        dirty="$dirty (sync to $(git -C "$dir" rev-parse --short=7 MERGE_HEAD) prepared)"
    fi
